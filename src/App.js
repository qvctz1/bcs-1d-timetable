...
const SHEET_URL =
  "https://docs.google.com/spreadsheets/d/1qL0Q5KVq3V1_fmprm9sAMz97a8P3wXjxVUpCrZbXRTM/export?format=csv&gid=1358384554";
...
// Define breaks after specific time slots
const BREAKS = ["09:45", "11:35", "01:25", "02:20"];
...
export default function App() {
  ...
  useEffect(() => {
    Papa.parse(SHEET_URL, {
      ...
      header: true,
      complete: (result) => {
        // Filter only BCS-1D rows
        const filtered = result.data.filter(
          (row) => row.Section?.trim().toLowerCase() === "bcs-1d"
        );
        setData(filtered);
      },
    });
  ...
  function getCell(day, time) {
    const lecture = data.find(
      (row) => row.Day === day && row.Time?.trim() === time
    );
    if (!lecture) return "";
