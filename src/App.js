...
return ( <div className="min-h-screen bg-gray-100 p-6"> <h1 className="text-3xl font-bold text-center mb-6">BCS-1D Timetable</h1> <div className="overflow-x-auto"> <table className="w-full border-collapse text-sm"> <thead> <tr> <th className="border p-2 bg-gray-200">Day</th> {TIMES.map((time) => ( <th key={time} className={border p-2 bg-gray-200 ${BREAKS.some((b) => time.includes(b)) ? "bg-yellow-100" : ""}} > {time} </th> ))} </tr> </thead> <tbody> {DAYS.map((day) => ( <tr key={day}> <td className="border p-2 font-semibold bg-gray-50">{day}</td> {TIMES.map((time) => ( <td key={time} className={border p-2 text-center whitespace-pre-line ${BREAKS.some((b) => time.includes(b)) ? "bg-yellow-50" : ""}} > {getCell(day, time)} </td> ))} </tr> ))} </tbody> </table> </div> </div> ); }
